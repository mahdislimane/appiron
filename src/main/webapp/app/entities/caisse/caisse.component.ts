...
export class CaisseComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.caisseService
        ...
        .subscribe(
          (res: HttpResponse<ICaisse[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  trackId(index: number, item: ICaisse): string {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInCaisses(): void {
    this.eventSubscriber = this.eventManager.subscribe('caisseListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ICaisse[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
